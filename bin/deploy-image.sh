...
helm upgrade --install --debug --dry-run ${RELEASE_NAME} ${CHART_PATH} --set image.repository=${IMAGE_REPOSITORY},image.tag=${IMAGE_VER},image.secretName="${CLUSTER_NAMESPACE}-us-icr-io",cluster_name="${CLUSTER_NAME}",region="${REGION}" --namespace ${CLUSTER_NAMESPACE}

echo -e "Deploying into: ${CLUSTER_NAME}/${CLUSTER_NAMESPACE}."
helm upgrade --install ${RELEASE_NAME} ${CHART_PATH} --set image.repository=${IMAGE_REPOSITORY},image.tag=${IMAGE_VER},image.secretName="${CLUSTER_NAMESPACE}-us-icr-io",cluster_name="${CLUSTER_NAME}",region="${REGION}" --namespace ${CLUSTER_NAMESPACE}
